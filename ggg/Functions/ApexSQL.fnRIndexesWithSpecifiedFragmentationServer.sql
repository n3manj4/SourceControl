...
-- Description:	Report on Indexes with fragmentation above xx% in specified SQL Server
-- =============================================
CREATE FUNCTION [ApexSQL].[fnRIndexesWithSpecifiedFragmentationServer]
(
...
(
	...
	(
		...
		WHERE M.SourceId IN (SELECT SourceId FROM [ApexSQL].[fnRGetAllIndexesForServer](@SourceId))
	) AS R 
	LEFT JOIN (SELECT * FROM [ApexSQL].[fnRGetAllIndexesForServer](@SourceId)) AS si ON
